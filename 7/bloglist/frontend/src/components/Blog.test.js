...
beforeEach(() => {
  ...
  container = render(
    <Blog blog={blog} user={user} updateBlog={addLike} />,
  ).container;
});

test("only title and author are rendered by default", () => {
  expect(container.querySelector(".hideDetails")).not.toHaveStyle(
    "display: none",
  );
...
test('URL and number of likes are shown when "view" button has been clicked', async () => {
  ...
  expect(container.querySelector(".showDetails")).not.toHaveStyle(
    "display: none",
  );
});
