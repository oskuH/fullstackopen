...
const favoriteBlog = (blogs) => {
  const reducer = (maxEntry, currentEntry) =>
    currentEntry.likes > maxEntry.likes ? currentEntry : maxEntry;
...
const mostLikes = (blogs) => {
  if (blogs.length !== 0) {
    ...
    const authorTotalLikes = _.mapValues(blogsByAuthor, (authorBlogs) =>
      _.sumBy(authorBlogs, "likes"),
    );

    const authorWithMostLikes = _.maxBy(
      _.entries(authorTotalLikes),
